 ...
 *
 * <h2>왜 언어 파일을 쓰지 않는가</h2>
 * <p>바닐라에도 {@code biome.minecraft.*} 번역 키가 있으므로
 * {@code Component.translatable} 한 줄이면 끝날 것처럼 보인다. 그러나 그 길은 <b>클라이언트의
 * 언어 설정을 따라간다.</b> 영어로 켜 둔 사람에게는 영어가 나오고, 그러면 「한글 바이옴 이름을
 * 늘 보여 준다」는 이 표시의 목적이 통째로 사라진다. 그래서 언어 설정과 무관하게 늘 같은
 * 글자가 나오도록 표를 코드 안에 박아 둔다.
 *
 * <p>표의 한글 이름은 <b>마인크래프트 26.2 공식 한국어 번역({@code ko_kr.json})에서 그대로
 * 옮겼다.</b> 임의로 지어낸 이름이 아니므로, 한국어로 하는 사람이 게임 안 다른 곳에서 보던
 * 이름과 어긋나지 않는다.
 *
 * <h2>왜 리소스 파일이 아니라 상수 Map 인가</h2>
 * <p>리소스로 빼면 리소스팩·데이터팩 로딩 순서와 얽히고, 파일이 빠지면 조용히 빈 이름이
 * 나온다. 66줄짜리 표 하나 때문에 그 위험을 질 이유가 없다. 코드 안에 있으면 컴파일이
 * 보증해 주고 단위 시험으로 통째로 훑을 수 있다.
 *
 * <h2>모르는 id 에서 터지지 않는다</h2>
 * <p>다른 모드가 넣은 바이옴이나 다음 판올림에서 늘어난 바이옴은 표에 없다. 그때는 예외를
 * 던지지도, 빈칸을 남기지도 않고 <b>id 의 path 부분을 그대로</b> 보여 준다. 「무엇인지 모르지만
 * 이런 이름이다」가 「아무것도 안 나온다」보다 언제나 낫고, HUD 는 매 프레임 돌기 때문에
 * 여기서 예외가 나면 화면 전체가 죽는다.
 */
public final class BiomeNames {
	 ...
	 * 전부 담았다. 순서는 그 클래스에 적힌 차례와 같게 두어, 판올림 때 새 바이옴이 어디에
	 * 끼어들었는지 눈으로 대조할 수 있게 했다.
	 */
	 ...
	 * 돌려준다. <b>빈 문자열은 「그 줄을 그리지 말라」는 뜻이다</b> — 「알 수 없음」 같은 글자를
	 * 대신 적으면, 청크가 아직 안 온 한두 프레임 동안 화면에 그 글자가 번쩍인다. 차라리 한
	 * 프레임 비어 있는 편이 눈에 덜 거슬린다.
	 */
