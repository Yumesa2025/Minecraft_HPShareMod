 ...
 * 떨어지는 중인지도 따지지 않는다. "뗐다가 다시"를 요구하는 것은 바닐라가 땅에서 점프한 그
 * 틱 안에 이미 땅을 떠나기 때문이다 — 그 조건을 빼면 스페이스 한 번이 땅 점프와 공중 점프를
 * 함께 써 버려 두 번째 점프가 아예 보이지 않는다.
 *
 * <h2>미는 것은 클라이언트다</h2>
 * <p>누른 그 틱에 {@link AirJumpImpulse} 가 낸 속도를 스스로 싣는다. 서버가 속도를 내려보내
 * 주기를 기다리면 그 왕복 동안(빨라야 두 틱, 핑이 붙으면 더) 몸이 계속 떨어진다.
 *
