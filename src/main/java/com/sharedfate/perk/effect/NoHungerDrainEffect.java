 ...
 * {@code Player.causeFoodExhaustion} 에서 배율을 0 으로 만들 뿐이다.
 *
 ...
 * <p>이 필드를 참으로 두면 {@link com.sharedfate.perk.PerkFoodRules#addNaturalRegenExhaustion}
 * 이 자연 회복의 대가도 건너뛴다. 이 필드가 없는(또는 거짓인) 다른 팀에는 위 기본 규칙이
 * 그대로 적용된다 — 이 타입을 나중에 다른 증강이 가져다 써도 자연 회복까지 공짜가 되는 사고가
 * 저절로 일어나지 않는다.
 *
 * <p>{@link PerkEffect#apply} 로 팀원에게 붙일 것이 없다. 소모도가 쌓이는 한가운데서 "이 팀이
 * 이 효과를 갖고 있는가", "자연 회복까지 면제하는가"만 물어본다.
 * {@code includeNaturalRegen} 이 거짓인 흔한 경우에는 여러 증강이 {@link #INSTANCE} 하나를
 * 돌려써도 안전하다.
 */
