...
/**
 * <b>이번 회차에만</b> 다시 뽑기 횟수를 더 준다. 세트 「도박 2단계 — 한 판 더」가 쓴다.
 *
 * <pre>{@code
 ...
 * 접속할 때마다 5씩 불어난다({@code item_grant} 를 그 자리에 두면 안 되는 것과 같은 이유다).
 *
 * <p>거기서도 「5를 더한다」가 아니라 {@link TeamState#syncRerollSetBonus} 로 <b>「세트로 얻은
 * 몫을 지금 값에 맞춘다」</b> 를 시킨다. 이미 5를 받아 둔 팀은 다시 불러도 아무 일이 없고, 세트가
...
public final class ExtraRerollsEffect implements PerkEffect {
	...
	public static int bonusOf(@Nullable TeamState state) {
		...
		// 이 한 줄이 없으면 빌드도 통과하고 로그도 없는데 도박 2단계만 완전 무동작이 된다.
