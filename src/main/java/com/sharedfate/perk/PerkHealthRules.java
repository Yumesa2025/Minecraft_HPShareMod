 ...
 *
 * <h2>고정이 보너스를 이긴다</h2>
 * <p>{@code max_health_lock}(고행자)이 있으면 보너스는 통째로 무시된다. 사용자가 그렇게
 * 정했고, 작성표의 "다른 증강으로도 오르지 않는다"와도 맞는다. 이 우선순위를 정하는 자리는
 ...
 * 바닐라가 플레이어의 현재 체력을 10 으로 자르고, {@code StatMirror} 는 그 감소를 이번 틱의
 * 피해로 관측해 공유 체력에서 뺀 뒤 새 상한으로 다시 자른다. 여기서 우리가 공유 체력까지 미리
 * 깎아 두면 <b>같은 감소가 두 번</b> 들어가 공유 체력이 0 이 되고 팀이 전멸한다. 예전에 고친
 * 공유 상태이상 배수 버그와 정확히 같은 모양의 함정이다. 상한만 옮기면 관측과 자르기가 같은
 * 결론에 이르므로 결과는 언제나 "가득 찬 10" 이거나 "원래 값" 이다.
 *
 * <h2>증강을 쓰지 않는 팀이면</h2>
