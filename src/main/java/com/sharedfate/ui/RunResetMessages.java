...
public final class RunResetMessages {
	 ...
	 *
	 * <p><b>다시 하는 법을 반드시 적는다.</b> 여기까지 왔다는 것은 수락을 이미 받았다는 뜻이고,
	 * {@code RunResetCommand.confirm} 은 수락을 받아들이면서 대기를 비운다. 그래서 실패한
	 * 사람이 {@code /yes} 를 다시 쳐도 「기다리는 요청이 없습니다」만 본다 — 처음부터 다시
	 * 하라고 말해 주지 않으면 그 자리에서 막힌다.
	 */
	...
	/**
	 * 월드를 지울 수 없는 자리에서 친 경우 — 싱글플레이·LAN.
	 *
	 * <p>26.3 통합 서버는 월드를 {@code <실행 폴더>/saves/<월드>} 에 두는데, 이 명령이 쓰는 월드
	 * 표식 계약은 <b>서버 루트 바로 아래의 월드 폴더 하나</b>를 전제한다. 그래서 통합 서버에서는
	 * 표식을 쓸 자리가 없고, 있다 해도 그 표식을 읽어 월드를 지워 줄 루프 스크립트가 없다.
	 *
	 * <p><b>「다시 시도하세요」를 적지 않는다.</b> 설정이 아니라 서버를 띄운 방식 때문이라 몇
	 * 번을 쳐도 똑같이 거절당한다. 고칠 길을 적는 것이 맞다.
	 */
	public static String worldNotResettable() {
		return "이 서버는 월드를 지울 수 없는 자리에서 돌고 있어 아무것도 지우지 않았습니다."
				+ " 서버 초기화는 재시작 루프 스크립트로 띄운 전용 서버에서만 쓸 수 있습니다."
				+ " (싱글플레이·LAN 월드는 게임에서 직접 지워 주세요.)";
	}

	/**
	 * 지난 월드 초기화의 표식이 아직 남아 있는 경우.
	 *
	 * <p>이 파일이 남아 있으면 <b>루프 스크립트가 기동 자체를 거부한다</b> — 자동 재시도를 막는
	 * 장치다. 그 상태에서 초기화를 시작하면 팀만 잃고 서버는 그대로 돈다. 그러니 여기서 막되,
	 * 이름이 점으로 시작하는 숨김 파일이라 <b>경로를 그대로 적어</b> 준다.
	 *
	 * @param markerPath 남아 있는 월드 표식의 절대 경로
	 */
	public static String staleWorldMarker(String markerPath) {
		return "지난 월드 초기화의 표식이 남아 있어 아무것도 지우지 않았습니다."
				+ " 이 파일이 있는 동안에는 재시작 루프 스크립트가 서버를 켜지 않습니다: "
				+ markerPath + retryLine();
	}

	/**
	 * 「처음부터 다시 하세요」 한 줄.
	 *
	 * <p>되묻기는 수락을 받는 순간 사라진다. 고치면 다시 될 수 있는 실패에만 붙인다.
	 */
	private static String retryLine() {
		return " 고친 뒤 /" + RESET_COMMAND + " 부터 다시 입력하세요.";
	}
