 ...
 *
 * <p><b>이것은 임시 다리다.</b> 보유 목록이 유형 id 를 함께 실어 오면 이 계산은 필요 없어진다 —
 * 이름이 같은 증강이 둘 있으면 여기서는 가릴 수 없고, 이름표 목록이 상한
 * ({@code PerkSetSyncPayload.MAX_CATALOG}) 에 잘리면 뒤쪽 유형의 증강은 되짚기에서 빠지기
 * 때문이다. 되짚지 못하면 그 줄에는 툴팁이 안 뜰 뿐 아무것도 깨지지 않는다.
 *
 * <h2>유형이 여럿일 수 있다</h2>
 * <p>증강 하나가 유형을 여럿 가질 수 있어 이름표도 유형마다 한 줄씩 실린다. 그래서 하나가 아니라
 * <b>목록</b>을 돌려준다. 화면은 유형마다 툴팁 한 덩어리씩을 이어 붙인다 — 하나만 고르면 나머지
 * 유형의 진행도를 영영 못 보게 되는데, 그 증강을 고른 이유가 그쪽이었을 수 있다.
 */
